...
1. Read spec artifacts (required): `openspec/changes/{change-name}/specs/**/spec.md`
2. Read tasks artifact (required): `openspec/changes/{change-name}/tasks.md`
3. Read design artifact: `openspec/changes/{change-name}/design.md`
4. Read apply progress artifact: `openspec/changes/{change-name}/apply-progress.md`
5. Check completeness: all tasks done?
6. Run tests (detect runner from `openspec/config.yaml`, package.json, Makefile, etc.)
7. Run build/type check when available
...
10. Write the verify report to `openspec/changes/{change-name}/verify-report.md`
