...
2. **Scope** — offer Option A (Full Technical Wiki under `openwiki/`,
   recommended), Option B (Lightweight wiki under `docs/wiki/`), and Option C
   (custom freeform path).
...
Resolve the approved output directory from the resolved `scope_choice`:
...
1. Determine the approved output directory yourself, from the
   `scope_choice`/`custom_path` values already resolved at launch time
   (Section 3 above) — never by trusting the executor's self-report.
2. Run `git status` scoped to exactly: the resolved output directory, plus
   the two declared exceptions `/AGENTS.md` and `/CLAUDE.md`.
