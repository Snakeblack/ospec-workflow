...
Nota de estado del binding (2026-09-26): el primer slice de este change existe como adaptador aislado en [operation-identity-binding](../../scripts/lib/operation-identity-binding.js), una función pura y opt-in que compara snapshots explícitos —binding `{changePath, phase, expectedRevision, operation}` contra snapshot objetivo y/o registros candidatos del mismo `changePath`— y devuelve una señal tipada (`bound`, `rejected`, `reconciliation_required`) sin autorizar ni invocar ningún efecto. Sus tests demuestran las señales de mismatch (change, phase, revision, operation y binding malformado), de ambigüedad (snapshots contradictorios y registros conflictivos del mismo `changePath` fallan cerrados, sin asignación por recencia) y de outcome registrado (success, failure, unknown o no reconocido producen `reconciliation_required`, nunca autorización ni reintento), y que un `verify` exitoso no equivale a PASS funcional. Límites deliberados y vigentes: no existe canal confiable de dispatch a SubagentStop, ni paridad con Go del adaptador, ni enforcement vivo; `reconciliation_required` es solo una señal, sin reconciliación durable implementada, y queda a cargo de reconciliación externa. El follow-up 1 se redujo honestamente a observabilidad: al no existir una clave cruzada atestada entre PreToolUse (`tool_use_id`) y SubagentStop (`agent_id`) en el contrato documentado del host, `subagent-stop` en Node (proyección de envelope, coste de fase y medición CX0) y en Go (proyección y coste de fase) falla cerrado ante un conjunto ambiguo de changes activos —ya no elige por recencia de `state.yaml`— y registra un único evento de auditoría global con `authentication: none`, sin nombre de change adivinado; con exactamente un change activo se conserva el flujo legacy y la salida del hook. Esto sigue sin ser identidad confiable: cualquier atribución exige reconciliación externa y el binding no está activo como canal atestado; el roadmap no pasa a «implemented».
