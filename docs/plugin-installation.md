...
| `codex` | Genera `.codex-plugin/plugin.json` y `.codex/agents/*.toml`. La instalación usa `dist/codex-marketplace/` para registrar el plugin y copia por separado solo `.codex/agents/`, sin modificar configuraciones existentes. | `scripts/configure/validate-codex.js`, ejecutado por la validacion de perfiles y por `node scripts/check.js`. |
...
- **Codex CLI (Global)**: compila `dist/codex/`, genera `dist/codex-marketplace/`, registra el marketplace local y copia `.codex/agents/*.toml` a `~/.codex/agents/` sin modificar `~/.codex/config.toml`.
...
4. Deja `~/.codex/config.toml` sin cambios.
...
3. Conserva cualquier `config.toml` global sin cambios.
