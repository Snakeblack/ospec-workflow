...
> **Versión de referencia:** v2.29.1, 2026-07-16.
...
| Siguiente | G0 — gobernanza documental | Instalar este sistema documental en rutas versionadas y archivar fuentes antiguas |
| Después | O6 — archive determinista | Cierre transaccional sin depender del modelo para operaciones mecánicas |
...
- Refactors puramente editoriales pueden usar direct + reconcile, salvo que cambien autoridad o contrato; G0 cambia gobernanza y debe dogfoodearse.
...
1. Terminar y archivar el change local de O4+O5 sin incorporar esta migración documental en su diff.
2. Aplicar **G0** mediante un change independiente.
3. Continuar la ruta crítica descrita en las olas siguientes.

### 3.3 Change G0 recomendado

```yaml
change: unify-harness-evolution-governance
intent: documentation-governance
...
- añadir `docs/architecture/harness-evolution.md`;
- añadir `docs/roadmaps/README.md` y `docs/roadmaps/harness-evolution.md`;
- mover roadmaps de target a `docs/roadmaps/targets/`;
- archivar los análisis anteriores bajo `analisis-fino/archive/` o una ruta histórica equivalente;
- eliminar los documentos activos duplicados después de verificar enlaces e inventario;
- actualizar cualquier referencia interna a los paths nuevos;
- decidir si `analisis-fino/archive/` se versiona o permanece local; los documentos activos sí deben quedar tracked.

Done criteria:

- una sola fuente conceptual y una sola operativa;
- ningún enlace activo apunta a `plan-siguiente-sesion.md` o a roadmaps históricos;
- los roadmaps de target declaran dependencia del general;
- grep del repositorio no encuentra una segunda declaración de “fuente de verdad operativa”;
- inventario origen/destino verificado antes de borrar;
- `docs-lint`, tests de enlaces y checks del repositorio pasan;
- archive report registra todos los archivos movidos.
...
#### G0. Gobernanza documental unificada — **pending** ▶ SIGUIENTE tras O4+O5

Dependencias: O4+O5 archivado.

Resultado: instalar los documentos de este paquete y retirar autoridades paralelas.

### Ola 1 — seguridad determinista y perfil adaptativo

#### O6. Archive como runtime determinista — **pending**
