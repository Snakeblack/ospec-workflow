...
- `agents` domain spec — how capabilities gate stack-skill injection into sub-agent
...
reason criteria only — it MUST NOT create a new phase, gate, artifact, or
...
- Both runtimes MUST enforce non-empty string constraints (`minLength: 1`, non-whitespace) on `assumptions` fields (`id`, `phase`, `statement`, `basis`).
- Both runtimes MUST strictly validate the type of `detailed_report` when present (`typeof === "string"` in JS, string type assertion `s, ok := v.(string)` in Go), rejecting invalid non-string types such as numbers, booleans, arrays, or objects.
...
- Both test suites MUST load the complete matrix of shared fixtures from `schemas/kernel/result-envelope/v1/fixtures/` (both `valid/` and `invalid/` directories), including `whitespace-only-required-strings.json` and `non-string-detailed-report.json`.
...
(Previously: REQ-skills-018 did not require strict runtime string type validation for detailed_report in JS and Go, nor differential parity across whitespace-only strings and non-string detailed_report fixtures.)
...
#### Scenario: JS and Go validators enforce string type on detailed_report when present

- GIVEN a result envelope payload containing `detailed_report` with a non-string value (such as a number, boolean, or array)
- WHEN validated using JS (`scripts/lib/result-envelope.js`) or Go (`internal/resultenvelope/resultenvelope.go`) validators
- THEN validation MUST fail closed in both runtimes identifying the invalid type for `detailed_report`

#### Scenario: Automated differential conformance validation across all shared fixtures in Node and Go

- GIVEN the shared fixture catalog in `schemas/kernel/result-envelope/v1/fixtures/` containing both valid and invalid fixtures, including `whitespace-only-required-strings.json` and `non-string-detailed-report.json`
...
#### Scenario: Differential conformance rejects whitespace-only strings and non-string detailed_report across all runtimes

- GIVEN the shared fixtures `whitespace-only-required-strings.json` and `non-string-detailed-report.json`
- WHEN evaluated by JSON Schema, JS validator, and Go validator
- THEN `schema.valid` MUST be `false`
- AND `js.valid` MUST be `false`
- AND `go.valid` MUST be `false`
