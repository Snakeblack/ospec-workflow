...
- Both runtimes MUST validate `question_gate` structural requirements (`reason` string, `questions` array of objects with `header`, `question`, and `options` array).
- Both runtimes MUST validate `verify_outcome` against `["PASS", "PASS WITH WARNINGS", "FAIL"]` when present.

(Previously: REQ-skills-018 did not mandate verify_outcome emission for sdd-verify, and validators lacked strict parity for Go schema_version checking, array item string types, skill_resolution enum enforcement, and question_gate structure.)
