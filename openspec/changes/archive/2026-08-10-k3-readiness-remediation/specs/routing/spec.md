...
### 16.4 Frozen Review Genesis and Slice-Scoped Targeted Correction {#REQ-routing-004}
...
### 16.5 Read-Only Gate Continuation, Migration, and Interruption Recovery {#REQ-routing-005}
...
## 17. Strict TDD Evidence-Only Remediation Routing
...
The 4R gate persistence model MUST retain every lineage generation additively,
with an unambiguous active-lineage reference and an explicit predecessor link for
each successor. Creating or activating a successor MUST preserve the complete,
byte-equivalent terminal predecessor record, including its candidate, genesis
paths, findings, budgets, executions, pending/unknown history, slices, and
terminal outcome. Readers MUST resolve the active lineage and its ordered
predecessor chain deterministically without overwriting a prior generation.

Before any mutable successor action, a legacy singular `lineage` representation
MUST migrate additively and idempotently into the generational representation.
The migration MUST preserve all legacy data literally and MUST fail closed on
collision, ambiguous active selection, malformed chain, pending/unknown state, or
partial-write detection. A pending persistence mutation MUST be durably recorded
before dispatch; unknown outcomes permit only exact reconciliation. Downstream
verify, delivery, and archive checks MUST be read-only and MUST validate both the
active lineage and its chain without allocating reviewers, findings, executions,
budgets, or successor authority.

This requirement MUST NOT introduce K4a Graph, Obligation Manifest, replay, or
worker-authority behavior, and MUST NOT modify historical archive evidence.

#### Scenario: Create and persist successor preserves predecessor

- GIVEN a terminal predecessor and an approved distinct successor request
- WHEN the gate creates and persists the successor
- THEN the predecessor record MUST remain complete and byte-equivalent
- AND the active reference MUST identify only the new successor with its link

#### Scenario: Second successor retains a deterministic chain

- GIVEN a terminal first successor in a persisted two-generation chain
- WHEN an independently approved second successor is created
- THEN all earlier generations MUST remain preserved without overwrite
- AND readers MUST resolve the active lineage and ordered chain deterministically

#### Scenario: Legacy singular lineage migration is idempotent

- GIVEN a legacy gate with one singular terminal lineage and no pending mutation
- WHEN mutable successor processing begins twice with identical input
- THEN the first run MUST add the generational representation without altering legacy data
- AND the second run MUST produce no duplicate generation or changed predecessor data

#### Scenario: Collision or partial persistence fails before dispatch

- GIVEN a generation id collision, ambiguous active reference, partial write, or pending/unknown outcome
- WHEN the gate prepares a successor mutation
- THEN it MUST fail closed or reconcile exactly before any reviewer dispatch
- AND existing findings, budgets, executions, and predecessor history MUST remain preserved

#### Scenario: Downstream gates remain read-only across generations

- GIVEN a valid active successor and its terminal predecessor chain
- WHEN verify, delivery, or archive checks run
- THEN they MUST validate the active lineage and chain without mutating review state
- AND they MUST NOT allocate a reviewer, finding, execution, budget, or successor
