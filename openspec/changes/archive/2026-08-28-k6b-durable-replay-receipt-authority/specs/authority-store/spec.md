...
When two writers supply the same `expectedRevision`, at most one CAS MUST succeed. The loser MUST receive a stable `cas-conflict`, MUST NOT restart work, inflate budgets, or silently apply `nextState`. `commitJournal` across `AuthorityStore`, `MemoryStore`, and `FileSystemStore` MUST merge and deduplicate by `effect_id`, preserving peer `mid_op_ticket` instances keyed by token, revision, and state digest. Journal status merge MUST be monotonic: an existing `completed` effect MUST NOT be replaced by stale `planned`, `started`, `failed`, or `unknown` data. A winning CAS MUST remove only its own ticket; a loser MUST retain durable journal evidence and its runtime-owned carry-over.

(Previously: Merge-safe upsert by `effect_id` did not prohibit a stale journal status from degrading `completed`.)

#### Scenario: Concurrent writers race on same revision

- GIVEN two writers load revision R for subject S
- WHEN both call `compareAndSwap(S, R, …)`
- THEN exactly one MUST succeed and the other MUST return `cas-conflict`
- AND the conflict MUST NOT alter budgets

#### Scenario: Concurrent commitJournal preserves peer tickets

- GIVEN writers W1 and W2 receive tickets T1 and T2 for R0
- WHEN W1 commits with T1
- THEN T1 MUST be deleted only after its winning CAS
- AND T2 MUST remain available to W2

#### Scenario: Winning CAS deletes only its own ticket

- GIVEN active tickets T1 for W1 and T2 for W2
- WHEN W1 wins `compareAndSwap` with T1
- THEN the store MUST delete T1
- AND MUST preserve T2 for W2 reconciliation

#### Scenario: Stale journal update cannot degrade completed

- GIVEN committed journal entry `eff-101` has status `completed`
- WHEN a stale writer upserts `eff-101` with `started` or `failed`
- THEN the committed entry MUST remain `completed`
- AND its completed result evidence MUST remain intact

#### Scenario: Journal merge retains distinct effects

- GIVEN existing `eff-101` and an incoming distinct `eff-102`
- WHEN `commitJournal` merges the payload
- THEN both effect IDs MUST appear once
- AND no duplicate record MAY be committed

### Requirement: Unified Atomic CAS Record {#REQ-authority-store-011}

The `AuthorityStore` MUST commit state, journal, authority bag, and budgets as one atomic CAS record. Successful CAS MUST update all four together. Journal entries MUST be deduplicated by `effect_id` and merged with the same monotonic status rule: `completed` MUST remain completed against stale lower-progress writes. Authority and budgets MUST NOT be detached post-CAS. A successful CAS MUST remove only the matching `midOpTicket` while preserving concurrent peer tickets.

(Previously: Atomic journal merging required deduplication by `effect_id` but did not make `completed` non-degradable.)

#### Scenario: Single atomic CAS record commit

- GIVEN a valid mutation payload containing state, journal, authority, and budgets
- WHEN `compareAndSwap` succeeds
- THEN the winning revision MUST contain all four components consistently

#### Scenario: Atomic commit preserves peer ticket

- GIVEN tickets T1 and T2 for concurrent writers
- WHEN W1 wins CAS with T1
- THEN the store MUST remove T1 and preserve T2

#### Scenario: Atomic CAS retains completed against stale journal

- GIVEN the committed journal marks `eff-101` completed
- WHEN CAS receives a stale `eff-101` status below completed
- THEN the committed journal MUST retain completed
- AND MUST not duplicate `eff-101`

### Requirement: Additive Runner Receipt CAS Collection {#REQ-authority-store-018}

The Authority Store MUST persist `runner-receipt/v1` records in an additive CAS collection distinct from OperationReceipt receipts. The public field name of that collection is design-owned. The collection MUST accept only records whose `kind` is `runner-receipt/v1`. A `runner-receipt/v1` record MUST NOT be stored as an OperationReceipt, and an OperationReceipt MUST NOT be stored in the runner-receipt collection. Kind mismatch MUST fail closed and MUST NOT advance the head.

Existing `authority.receipts` OperationReceipt behavior MUST remain unchanged. Reloading the store from disk MUST restore the runner-receipt collection together with the authority bag, journal, and budgets, without manual snapshot extraction. This extends restart preservation; REQ-013 is unchanged.

#### Scenario: Runner-receipt records persist in a distinct CAS collection

- GIVEN a successful CAS that includes schema-valid `runner-receipt/v1` records
- WHEN the winning revision is stored
- THEN those records MUST reside in an additive CAS collection distinct from OperationReceipt receipts
- AND `authority.receipts` MUST still contain only OperationReceipt values

#### Scenario: Kind mismatch with OperationReceipt fails closed

- GIVEN a CAS payload that places a `runner-receipt/v1` record among OperationReceipt receipts, or an OperationReceipt in the runner-receipt collection
- WHEN `compareAndSwap` is evaluated
- THEN the call MUST fail closed
- AND the head revision MUST remain unchanged

#### Scenario: Restart restores runner-receipt records without mixing families

- GIVEN a committed head that stores OperationReceipt receipts and `runner-receipt/v1` records in distinct collections
- WHEN the process terminates and a new store instance loads from the same disk location
- THEN both collections MUST be fully restored
- AND recovery MUST NOT require invoking manual `snapshot()` extraction
- AND the restored runner-receipt collection MUST NOT contain OperationReceipt values
