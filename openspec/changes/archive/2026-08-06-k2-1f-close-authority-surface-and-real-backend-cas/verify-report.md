...
### REQ-authority-store-020 — Lockfile Owner Token & Safe Unlink / Atomic Stale Lock Takeover

**Status: PASS**

Evidence:
- [`filesystem-store.js` L28–33](file:///c:/Users/sn4ke/dev/activos/ospec-workflow/scripts/lib/filesystem-store.js#L28-L33): `withFileLock` generates `ownerToken = randomUUID()` and writes JSON `{ ownerToken, pid: process.pid, timestamp: Date.now() }` to `.lock`.
- [`filesystem-store.js` L48–73](file:///c:/Users/sn4ke/dev/activos/ospec-workflow/scripts/lib/filesystem-store.js#L48-L73): Stale lock inspection reads lock payload and verifies `!isPidAlive(lockData.pid)` before executing atomic handle-bound takeover (`"r+"` open handle, token match under handle, `truncate(0)` + `writeFile` + `sync`). Eliminates TOCTOU race on stale lock removal.
- [`filesystem-store.js` L80–87](file:///c:/Users/sn4ke/dev/activos/ospec-workflow/scripts/lib/filesystem-store.js#L80-L87): `finally` block reads lockfile content, parses JSON, and only unlinks if `lockData.ownerToken === ownerToken`.
...
- [`export-surface.test.js`](file:///c:/Users/sn4ke/dev/activos/ospec-workflow/scripts/lib/lifecycle-kernel/export-surface.test.js): Tests verify `_internalCreateIssuer`, `mintOperationPermit`, `issueOperationPermit`, `isPermitAuthorityIssuer`, `runKernelOperation`, `PERMIT_AUTHORITY_ISSUER`, and `createPermitAuthorityIssuer` are all `undefined` on production `require()`. Also asserts `runtime.permitIssuer` is `undefined` on runtime instances.
...
Evidence:
- [`test-support/permit-test-helpers.js`](file:///c:/Users/sn4ke/dev/activos/ospec-workflow/scripts/lib/test-support/permit-test-helpers.js): Exports `createTestKernelRuntime`, `createTestPermitIssuer`, `mintTestPermit`, `issueTestPermit`, `issueFixturePermit`, `withRuntimePermit` for unit test use only. Imports from `../lifecycle-kernel/internal/permit-authority.js`.
- NOT re-exported from `lifecycle-kernel/index.js` or `permits.js` production surfaces.

### REQ-lifecycle-kernel-027 — Strict Internal Permit Authority Issuer Isolation

**Status: PASS**

Evidence:
- [`lifecycle-kernel/index.js` L608–620](file:///c:/Users/sn4ke/dev/activos/ospec-workflow/scripts/lib/lifecycle-kernel/index.js#L608-L620): `createKernelRuntime` does NOT accept `options.permitIssuer` and does NOT expose a `permitIssuer` getter. `KernelRuntime.runOperation` destructures input to discard `input.permitLedger` and enforces private `permitIssuer`.
- [`minimal-kernel-harness.js` L35–40](file:///c:/Users/sn4ke/dev/activos/ospec-workflow/scripts/lib/minimal-kernel-harness.js#L35-L40): `minimal-kernel-harness` uses `createTestKernelRuntime` for fault matrix testing without polluting production `createKernelRuntime`.
- [`lifecycle-kernel/index.test.js` L414–440](file:///c:/Users/sn4ke/dev/activos/ospec-workflow/scripts/lib/lifecycle-kernel/index.test.js#L414-L440): Unit tests assert `options.permitIssuer` is ignored during `createKernelRuntime` construction, `runtime.permitIssuer` is `undefined`, and rogue caller-supplied `permitLedger` is rejected with `permit-not-runtime-issued`.
...
| Phase 6 — Strict Issuer Isolation & Atomic Takeover | `AssertionError: runtime instance must not expose permitIssuer property` | Removed options.permitIssuer and getter, added atomic takeover in withFileLock | Added createTestKernelRuntime | ✅ |
...
0 errors, 0 warnings
All checks passed. (2020 tests passing)
```
