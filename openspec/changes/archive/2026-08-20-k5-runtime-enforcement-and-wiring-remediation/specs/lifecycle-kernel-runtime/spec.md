...
Lifecycle reducers, mutation processors, and `runKernelOperation` MUST enforce strict budget monotonicity and unified exhaustion checks via `isBudgetExhausted()`. Across retries, recovery loops, and CAS conflict reconciliations, remaining node quotas (`turns`, `patches`, `commands`, `wall_time_minutes`, `changed_lines`, `allowed_paths`) and authority/effect limits (`effect_attempts`, `authority_mutations`, `evidence_runs`, `review_sweeps`) MUST be strictly monotonically non-increasing. When any budget quota is exhausted (equals 0), the reducer and runtime MUST flag the entity as exhausted (`exhausted: true`), prune ordinary execution transitions, and apply post-effect zero-delta decrements before CAS commit. Reducers and runtime wrappers MUST NOT reset, restore, or silently replenish consumed budgets during state transitions or retry attempts.
(Previously: Reducer budget enforcement lacked direct integration with unified isBudgetExhausted evaluation and post-effect zero-delta accounting.)
