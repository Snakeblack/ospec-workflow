...
The execution system MUST define and enforce uniform execution budget quotas for every graph node across six orthogonal dimensions: `turns` (integer > 0), `patches` (integer >= 0), `commands` (integer >= 0), `wall_time_minutes` (number > 0), `changed_lines` (integer > 0), and `allowed_paths` (array of string glob patterns). The unified budget evaluator `isBudgetExhausted()` MUST evaluate all declared node budget dimensions; if ANY single dimension quota reaches zero or is exceeded by consumption, the node MUST be evaluated as exhausted (`exhausted: true`). A node execution that exceeds or exhausts any declared quota MUST be stopped immediately and pruned from further normal execution transitions.
(Previously: Node budget evaluation did not unify full six-dimension checking in a canonical isBudgetExhausted evaluator.)

#### Scenario: Node turn budget reached zero triggers exhaustion in isBudgetExhausted

- GIVEN a graph node with a `turns` quota of 5 and consumed `turns` equal to 5
- WHEN `isBudgetExhausted()` evaluates the node budget
- THEN it MUST return `exhausted: true` identifying dimension `turns`
- AND the runtime MUST halt the node and mark it budget-exhausted
...
#### Scenario: Command quota exhaustion halts worker execution

- GIVEN a node whose consumed `commands` reaches its declared limit of 25
- WHEN `isBudgetExhausted()` evaluates the node state
...
The execution system MUST define and enforce authoritative effect quotas across four explicit dimensions: `effect_attempts` (integer > 0), `authority_mutations` (integer >= 0), `evidence_runs` (integer >= 0), and `review_sweeps` (integer >= 0). The unified budget evaluator `isBudgetExhausted()` MUST evaluate all four authority dimensions; if ANY single authority dimension reaches zero or exceeds quota, authority execution MUST be marked exhausted (`exhausted: true`). Authoritative actions that exceed their dedicated budget MUST fail closed, MUST NOT be granted operation permits, and MUST deterministically prune execution transitions in the selector.
(Previously: Authority budget checking was evaluated separately without unified exhaustion semantics across all four dimensions.)
...
#### Scenario: Effect attempts exhaustion evaluated by isBudgetExhausted

- GIVEN an authority budget where `effect_attempts` reaches 0
- WHEN `isBudgetExhausted()` evaluates the authority budget
...
- AND MUST record a zero-delta attempt event in the journal
