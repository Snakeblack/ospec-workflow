...
| Script | Role |
|--------|------|
| `scripts/lib/review-dimensions.js` | quality review evidence normalization |
...
## Generator Runtime Inventory (merged)
...
### Requirement: Canonical Presets, Catalogs, and Model Capabilities {#REQ-generator-015}

`models.yaml` MUST remain the authoritative static source for target model defaults,
preset assignments, catalog entries, and model capabilities. Presets MUST identify
only known phases/agents and catalog choices; catalog entries MUST use target-scoped
opaque IDs or values and MAY declare supported `effort`, `model_reasoning_effort`,
`variant`, and runtime capabilities. The parser and policy validator MUST reject
duplicate keys, malformed capability values, unknown target references, and preset
references that cannot resolve. A catalog entry retained for Antigravity MUST NOT by
itself make Antigravity selectable or cause model metadata injection.

#### Scenario: Valid preset and capability data is accepted

- GIVEN `models.yaml` contains presets and target catalog entries that resolve to known phases and targets
- WHEN model policy validation runs
- THEN validation MUST succeed
- AND the adapter/generator MUST be able to retrieve the declared capabilities without mutating the source

#### Scenario: Broken preset reference fails closed

- GIVEN a preset references an unknown phase, target, or catalog choice
- WHEN the configure policy is validated
- THEN validation MUST return a structured error
- AND no generated output tree MUST be written

#### Scenario: Antigravity catalog does not enable selection

- GIVEN `models.yaml` contains informational Antigravity catalog values but no selectable profile capability
- WHEN the target plan and generator run
- THEN the plan MUST mark Antigravity as inherited
- AND generated Antigravity files MUST omit model-selection metadata

### Requirement: Target-Compatible Reasoning Metadata and Stale-Value Safety {#REQ-generator-016}

The generator MUST validate every selected model metadata object against the target
profile and the model's declared capabilities before transformation. It MUST emit only
supported fields: Claude `effort`, Codex `model_reasoning_effort`, OpenCode `variant`,
or other fields explicitly declared by a target profile. Unsupported or invalid values
MUST fail closed before writes. Default or absent reasoning values MUST clear stale
configuration according to the target contract; in particular an OpenCode assignment
without a variant MUST emit an explicit empty variant when deep merging could retain a
previous value. Targets without a supported reasoning field MUST omit it rather than
leaking it into generated frontmatter/configuration.

#### Scenario: Claude emits a supported effort

- GIVEN a Claude model declares `effort: high` as supported
- WHEN the Claude target is generated
- THEN the agent frontmatter MUST contain `effort: high`

#### Scenario: Codex emits its native reasoning field

- GIVEN a Codex choice contains a supported `model_reasoning_effort`
- WHEN the Codex target is generated
- THEN the generated agent configuration MUST contain `model_reasoning_effort`
- AND it MUST NOT invent an OpenCode `variant` field

#### Scenario: OpenCode clears a stale variant

- GIVEN the selected OpenCode model declares no variant while a prior configuration contains `variant: high`
- WHEN the OpenCode target is generated
- THEN the generated assignment MUST clear the prior value with the target's explicit default representation

#### Scenario: Unsupported reasoning value aborts generation

- GIVEN a selected model requests an effort, reasoning effort, or variant not declared compatible
- WHEN configure validates the model policy
- THEN it MUST return non-zero with a structured validation error
- AND MUST NOT write transformed files

#### Scenario: Unsupported fields do not leak to targets

- GIVEN a model object contains metadata for another target's control
- WHEN a target that does not declare that control is generated
- THEN the unrelated field MUST be absent from the emitted output
