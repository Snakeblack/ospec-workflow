...
Lifecycle reducers, mutation processors, and `runKernelOperation` MUST enforce strict budget monotonicity and unified exhaustion checks via `isBudgetExhausted()`. The runtime MUST evaluate budget exhaustion in preflight prior to invoking `effectExecutor`; if any node or authority quota is exhausted for non-terminal operations, execution MUST fail closed with exactly zero calls to `effectExecutor`. Terminal control transitions (`escalate`, `stop`) MUST be exempt from preflight budget exhaustion blocking, allowing them to consolidate and commit terminal state via CAS. Across retries, recovery loops, and concurrent multi-writer CAS conflict reconciliations, remaining node quotas (`turns`, `patches`, `commands`, `wall_time_minutes`, `changed_lines`, `allowed_paths`) and authority/effect limits (`effect_attempts`, `authority_mutations`, `evidence_runs`, `review_sweeps`) MUST be strictly monotonically non-increasing. When an effect is executed and the operation subsequently loses a CAS race, the consumed budget MUST be preserved via runtime-owned carry-over and deducted from the updated head revision without requiring caller-supplied fabricated arguments. Reducers and runtime wrappers MUST NOT reset, restore, or silently replenish consumed budgets during state transitions or retry attempts.
(Previously: Budget monotonicity did not support runtime-owned carry-over across multi-writer CAS races without fabricated arguments, nor did it explicitly allow terminal control transitions under budget exhaustion.)
...
#### Scenario: CAS reconciliation applies runtime-owned carry-over of consumed budget across lost CAS race

- GIVEN an operation that executed an effect and encountered a CAS conflict with 4 turns consumed
- WHEN the runtime reconciles and re-synchronizes the operation against the winning head revision
- THEN the next state transition MUST reflect the 4 consumed turns and decremented attempt quota via runtime-owned carry-over
- AND MUST NOT reset turn or attempt counters nor require caller-supplied fabricated arguments

#### Scenario: Preflight budget exhaustion halts non-terminal runKernelOperation with zero effect executor calls

- GIVEN a lifecycle state where `isBudgetExhausted()` returns `exhausted: true` for node or authority quotas
- WHEN `runKernelOperation` executes a non-terminal operation
- THEN the runtime MUST fail closed immediately
- AND MUST perform exactly zero calls to `effectExecutor`

#### Scenario: Terminal control transitions execute and commit via CAS even under budget exhaustion

- GIVEN a lifecycle state with exhausted node or authority budget
- WHEN `runKernelOperation` executes an explicit terminal control transition (`escalate` or `stop`)
- THEN the runtime MUST consolidate the terminal state and commit via CAS
- AND MUST NOT block preflight execution

#### Scenario: Reducer marks node exhausted when isBudgetExhausted triggers

- GIVEN a node whose consumed `commands` or `turns` reaches the declared quota limit
- WHEN the reducer processes the operation
- THEN `isBudgetExhausted()` MUST evaluate to `exhausted: true`
- AND the node MUST be flagged with `exhausted: true` preventing subsequent `start` or `recover` transitions
...
The lifecycle kernel runtime MUST record zero-delta attempt consumption strictly on effect-bearing code/file mutation operations (e.g. `repair`, `apply`, `mutate`) where `modifiedPaths` is empty or content diff is zero while the reducer outcome indicates no state advance (`reduced.outcome === "unchanged"`). Lifecycle transitions, control transitions (`escalate`, `stop`), and actions that produce genuine state progress (`reduced.outcome !== "unchanged"`) MUST NOT be penalized as zero-delta mutations even if zero source files were modified. When a zero-delta mutation is detected post-effect, the runtime MUST simultaneously decrement BOTH `node.turns` (or `node.budget.turns`) AND `state.authority_budget.effect_attempts`, and MUST persist a durable `zero-delta-attempt` event in the journal before committing via CAS. When any execution or authority budget reaches zero, the runtime MUST refuse normal execution transitions and transition deterministically to `escalate` or `stop`.
(Previously: Zero-delta accounting did not distinguish between effect-bearing code mutations without progress and legitimate lifecycle state advances or control transitions.)

#### Scenario: Zero-delta effect mutation decrements both turns and attempt counters with durable event

- GIVEN an authorized code mutation step that executes but results in unchanged state (`reduced.outcome === "unchanged"`) and zero modified files
- WHEN the runtime applies post-effect zero-delta accounting
- THEN `node.turns` AND `authority_budget.effect_attempts` MUST both decrement monotonically
- AND a durable `zero-delta-attempt` event MUST be recorded in the journal before CAS commit

#### Scenario: Lifecycle state advance without file changes is not penalized as zero-delta

- GIVEN an authorized lifecycle transition that produces state progress (`reduced.outcome !== "unchanged"`) with zero filesystem changes
- WHEN the runtime evaluates post-effect zero-delta conditions
- THEN the transition MUST NOT be penalized as a zero-delta mutation
- AND `authority_budget.effect_attempts` MUST NOT be dual-decremented

#### Scenario: Read-only diagnostics and control transitions are not penalized as zero-delta

- GIVEN a diagnostic inspection or terminal control transition (`escalate`, `stop`)
- WHEN post-effect accounting runs
- THEN the action MUST NOT be classified as a zero-delta attempt
