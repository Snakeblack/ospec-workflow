...
1. `inv-k5-budget-monotonicity`: verifies that no execution or authority budget increases across runtime operations, concurrent multi-writer CAS race reconciliations (with at least 2 competing writers where the losing writer's executed effect budget is preserved via 100% runtime-owned carry-over without caller-fabricated arguments and not restored upon retry), or repair loops under real store/CAS execution.
...
3. `inv-k5-allowlist-enforcement`: verifies that the runtime, boundary validator, and selector strictly emit and enforce allowlisted recovery transitions (`repair`, `replan`, `escalate`, `stop`) without unallowlisted operations or silent `decide` substitutions, emitting `operation: "repair"` for `code_defect`, harmonizing `kind`/`operation` taxonomies, enforcing boundary allowlists fail-closed, and committing `escalate` and `stop` to CAS as consolidated terminal outcomes even under budget exhaustion.
4. `inv-k5-zero-delta-consumption`: verifies that zero-delta mutations strictly decrement both node turns and authority `effect_attempts` monotonically within `runKernelOperation` for non-advancing code mutations and persist a durable `zero-delta-attempt` journal event before CAS commit, while legitimate state advances without file changes are not penalized.
5. `inv-k5-budget-exhaustion-terminal`: verifies that unified `isBudgetExhausted()` evaluation across all 6 node and 4 authority dimensions deterministically halts non-terminal execution in preflight with exactly zero calls to `effectExecutor` while allowing terminal control transitions (`escalate`, `stop`) to commit via CAS.
6. `inv-k5-honest-recovery-advancement`: verifies that recovery transitions advance the `blockingFingerprint` or terminate, enforcing mandatory fail-closed repair scope preflight and rejecting stagnant recovery loops fail-closed in runtime execution.
...
(Previously: Model invariants did not verify 100% runtime-owned carry-over in multi-writer CAS races without fabricated arguments, zero-delta exclusion for legitimate lifecycle progress, or terminal CAS commits under budget exhaustion.)
...
#### Scenario: Budget monotonicity verified across concurrent 2-writer CAS conflict race with runtime-owned carry-over

- GIVEN a model execution exploring a concurrent CAS race with 2 writers competing for the same head revision
- WHEN writer 1 commits and writer 2 fails with CAS conflict after executing an effect
- THEN the checker for `inv-k5-budget-monotonicity` MUST verify that writer 2 retains the decremented effect budget on retry through 100% runtime-owned carry-over without caller-fabricated arguments

#### Scenario: Zero-delta checker verifies dual decrement strictly for non-advancing effect mutations

- GIVEN a model execution producing a zero-delta mutation step
- WHEN invariant checker `inv-k5-zero-delta-consumption` runs
- THEN the checker MUST verify that both `node.turns` and `authority_budget.effect_attempts` were decremented
- AND MUST verify that a `zero-delta-attempt` event was recorded in the journal before CAS commit
- AND MUST verify that state progress without file changes is not penalized
...
#### Scenario: Exhausted budget terminality halts non-terminal operations while permitting terminal CAS commits
...
- THEN the checker MUST verify that non-terminal executions fail closed with zero calls to `effectExecutor`
- AND MUST verify that terminal `escalate` or `stop` transitions commit to the Authority Store via CAS
