...
The system MUST provide a deterministic replay engine that executes graph transitions against pre-recorded fixture results without instantiating or invoking live worker runtime authority. The replay engine MUST enforce `validateExecutionGraphBinding(graph)` at startup and fail closed if the graph structure or `GraphId` has been manipulated (`graph-id-mismatch`).

The canonical `replayExecutionGraph()` function MUST validate each recorded node result against the minimal canonical `ReplayFixtureResult` contract:
1. **Provenance**: Every recorded fixture MUST declare non-empty `graph_id` matching `graph.graph_id` byte-for-byte and non-empty `work_order_id` matching the deterministically compiled WorkOrder's `work_order_id`. Missing, empty, or mismatched `graph_id` or `work_order_id` MUST be rejected fail-closed with error code `stale-fixture-rejected`. Compiling WorkOrders during replay MUST fail closed without swallowing exceptions (`work-order-compilation-failed`).
2. **Terminal Status**: A node fixture result is completed if and only if `status` or `outcome` equals `"completed"`, `ok !== false`, neither `status` nor `outcome` equals `"cancelled"` or `"failed"`, and no contradictory status/outcome values exist.
3. **Exit Code**: If numeric `exit_code` is present, it MUST equal `0`. A non-zero `exit_code` with `completed` status or outcome is contradictory and MUST fail closed as a failed node.
4. **Evidence Object**: A completed node fixture MUST provide a non-null, non-array plain object `evidence` (`typeof recorded.evidence === "object"` and `!Array.isArray(recorded.evidence)`). Omission, null, array, or primitive evidence MUST fail closed.
5. **Node Required Evidence**: The replay engine MUST verify `node.required_evidence ⊆ Object.keys(recorded.evidence)`. If any required evidence identifier declared on the node is missing from `recorded.evidence`, the node MUST NOT be marked completed, MUST be treated as failed/unfulfilled, and MUST prevent dependent downstream nodes from completing.
6. **Obligation Satisfaction**: At graph level, all non-deferred contract `must` obligations MUST have all implementing nodes completed and all obligation `required_evidence` keys present in collected fixture evidence.
...
The replay engine MUST reject recorded fixtures for nodes invalidated by clarify events (`stale-fixture-rejected`). Replay MUST be idempotent: replaying identical valid fixtures MUST yield identical node outcomes, preserve obligation mappings, and avoid resurrecting invalidated nodes. Replay failure on an expected invariant MUST produce a reproducible counterexample trace.

#### Scenario: Fixture replay converges deterministically without live worker invocation

- GIVEN a valid Execution Graph and pre-recorded fixture results satisfying provenance, completed status, evidence objects, and required evidence
- WHEN `replayExecutionGraph()` executes
- THEN all graph nodes MUST complete deterministically with identical outcome state
- AND no live worker transport or runtime process MUST be invoked

#### Scenario: Fixture claiming completed without valid evidence object fails closed

- GIVEN a fixture declaring `status: "completed"`, matching `graph_id` and `work_order_id`, but omitting `evidence` or supplying null, array, or primitive evidence
- WHEN `replayExecutionGraph()` executes
...
- AND dependent downstream nodes MUST remain blocked
...
- AND the rejection MUST include the affected `node_id`
...
- AND the node MUST be classified as failed in the replay outcome
- AND downstream dependent nodes MUST remain blocked

#### Scenario: Replay fails closed on cancelled or malformed fixture results

- GIVEN a recorded fixture containing a node result with `status: "cancelled"`, `ok: false`, or conflicting status/outcome
- WHEN `replayExecutionGraph()` executes
...
- THEN fixtures for invalidated node IDs MUST be rejected fail-closed with `stale-fixture-rejected`
