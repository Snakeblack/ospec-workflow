...
The projector and replay MUST supply the Candidate's selected evidence strategy as the evidence-strategy binding of that integrity check, matching the verifier's binding for the same Candidate. Omitting the binding MUST NOT be treated as matching every plan strategy. A ChallengePlan that is internally canonical for a different strategy MUST fail closed with `GRAPH_DIVERGENCE`.
...
(Previously: projector and replay validated plan/result bindings to each other but did not require the Candidate's selected evidenceStrategy as an integrity-gate binding.)
...
#### Scenario: Wrong-strategy canonical K6c plan fails projection and replay

- GIVEN graph projection or replay whose Candidate selected evidence strategy is `feature`
- AND a schema-valid ChallengePlan whose `evidence_strategy` is `bug` with internally canonical passed results
- WHEN the projector or replay validates K6c material
- THEN it MUST fail closed with `GRAPH_DIVERGENCE`
- AND MUST NOT emit a graph
