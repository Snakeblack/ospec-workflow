...
- La enumeración de archivos preparados (`getStagedFiles`) MUST operar de forma fail-closed: si el subproceso `git diff --cached` falla o finaliza con código de salida distinto de cero, MUST lanzar un `Error` descriptivo en lugar de retornar un arreglo vacío.
- La lectura de contenido staged (`getStagedContent`) MUST operar de forma fail-closed: si `git show :<path>` falla o concluye con código distinto de cero para un archivo presente en el índice, MUST lanzar un `Error` descriptivo en lugar de retornar `null` o continuar.
- El hook pre-commit MUST abortar con código de salida distinto de cero si falla cualquier operación de acceso o consulta del índice de Git.
...
- WHEN se intenta enumerar los archivos preparados
- THEN `getStagedFiles` lanza un `Error` descriptivo del fallo
- AND el hook pre-commit aborta con código de salida no nulo impidiendo el commit.

#### Scenario: Fallo de Git al leer blob staged (fail-closed)
- GIVEN un archivo presente en el índice preparado pero cuyo comando `git show :<path>` falla o termina con código no cero
- WHEN `getStagedContent` intenta leer el contenido del blob
- THEN `getStagedContent` lanza un `Error` descriptivo
- AND el hook pre-commit aborta con código no cero impidiendo que el commit proceda a ciegas.
...
- Si los archivos preparados modifican únicamente instaladores o validadores de un target específico en `scripts/configure/` (e.g. `validate-cursor.js`, `install-codex.js`), el sistema MUST retornar únicamente dicho target.
...
- GIVEN un cambio preparado únicamente en `scripts/configure/validate-codex.js` o `scripts/configure/install-cursor.js`
- WHEN el sistema analiza los targets afectados
- THEN el resultado contiene exclusivamente el target correspondiente (`["codex"]` o `["cursor"]`)
- AND no se ejecutan regeneraciones para los targets no afectados.

#### Scenario: Fallback a ALL_TARGETS por cambio en generador compartido o librerías auxiliares
...
#### Scenario: Fallback a ALL_TARGETS por modificación en entradas canónicas del generador
- GIVEN un cambio preparado en `skills/branch-pr/SKILL.md`, `agents/sdd-spec.agent.md`, `commands/build.md`, `rules/code.md`, `hooks/pre-tool-use.json`, `schemas/kernel/action.json`, `.mcp.json` o `.claude-plugin/plugin.json`
- WHEN el sistema analiza los targets afectados
- THEN el resultado retorna la lista completa de todos los targets (`ALL_TARGETS`).

#### Scenario: Fallback a ALL_TARGETS por modificación en hooks distribuidos de runtime
- GIVEN un cambio preparado en `scripts/hooks/pre-commit-hook.js` o `scripts/hooks/session-start.js`
- WHEN el sistema analiza los targets afectados
- THEN el resultado retorna la lista completa de todos los targets (`ALL_TARGETS`).
