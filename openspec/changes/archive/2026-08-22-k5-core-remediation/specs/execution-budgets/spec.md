...
All node and authority budgets MUST be strictly monotonically non-increasing across execution retries, recovery loops, and concurrent CAS conflict reconciliations. When an operation executes an authoritative effect or incurs consumption and subsequently encounters a CAS conflict (such as losing an optimistic concurrency race against a concurrent writer in the Authority Store), the runtime MUST retain an exhaustive pending carry-over of consumed deltas across all declared dimensions: node dimensions (`turns`, `commands`, `patches`, `changed_lines`, `wall_time_minutes`, `allowed_paths`) and authority dimensions (`effect_attempts`, `authority_mutations`, `evidence_runs`, `review_sweeps`). The runtime MUST manage carry-over accumulation and replay automatically in a runtime-owned manner across retry cycles against the winning revision, preventing budget replenishment, quota resets, and effect duplication on retrying losers without requiring manual caller intervention or caller-supplied fabricated arguments.
(Previously: Monotonicity required preserving consumed budgets across CAS races, but did not specify exhaustive multi-dimensional tracking of all six node and four authority dimensions upon CAS conflict.)
...
- THEN the remaining budget MUST preserve the consumed attempt and 3 turns via runtime-owned carry-over
...
Zero-delta accounting MUST be strictly bounded to effect-bearing code and file mutation actions (such as `repair` or code patch operations) that fail to advance state (`reduced.outcome === "unchanged"` and zero modified lines or files). Such zero-delta mutation steps MUST consume an execution attempt and decrement BOTH node turn quotas (`node.turns`) AND authority `effect_attempts` simultaneously and monotonically, recording a durable `zero-delta-attempt` event in the journal before committing state via CAS. Operations that produce genuine lifecycle state progression (`reduced.outcome !== "unchanged"`), non-mutating inspections, read-only diagnostic steps, and terminal control transitions (`escalate`, `stop`) MUST NOT be penalized as zero-delta mutations even if zero filesystem modifications occur. All budget decrements MUST remain strictly monotonic and non-replenishing across retries and CAS reconciliations.
(Previously: Zero-delta accounting did not differentiate effect-bearing code mutations without progress from legitimate lifecycle state progression without file modifications.)
...
- WHEN the worker submits a patch that modifies zero lines or leaves file contents identical without advancing state (`reduced.outcome === "unchanged"`)
...
- GIVEN an authorized operation that advances semantic lifecycle state (`reduced.outcome !== "unchanged"`) without modifying filesystem files
