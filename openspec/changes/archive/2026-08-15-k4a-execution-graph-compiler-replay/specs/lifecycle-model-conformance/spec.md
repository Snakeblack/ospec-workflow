...
K2.1 MAY model subject- and budget-bound invalidation through opaque
values named `SubjectId` and `BudgetRef`. `OperationPermit` and
...
also be concrete enforceable artifacts under K2a. `PolicySnapshot` digests, SourceSnapshot provenance bindings, and Execution Graph compile/replay invariants MUST be concrete enforceable model structures under K4a. The model MUST NOT inspect or
invent the future internal structure of Candidate mutation, productive budget envelopes, live worker runtime authority, attestation or delivery authorization policy.
(Previously: PolicySnapshot and graph compile bindings remained future/opaque; K4a promotes PolicySnapshot, SourceSnapshot provenance binding, and Execution Graph compile/replay invariants to concrete model checks while Candidate mutation, live worker authority, and delivery policy remain opaque.)
...
correction budget monotonicity, delivery authorization, and live worker runtime execution. K2.1 Authority Store, OperationPermit/Receipt and
...
matrix invariants MUST NOT appear in the deferred list. K4a Execution Graph deterministic compile, SourceSnapshot provenance binding, PolicySnapshot digest binding, Obligation Manifest completeness, and fixture replay determinism invariants MUST NOT appear in the deferred list.
(Previously: deferred list excluded only K2.1 and K2a invariants; K4a promotes Execution Graph compile, SourceSnapshot provenance binding, PolicySnapshot binding, Obligation Manifest completeness, and fixture replay to non-deferred enforced invariants.)
