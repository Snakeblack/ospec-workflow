...
`challenge-plan/v1` MUST require: `schema_version`, `kind`, `plan_id` (`^sha256:[a-f0-9]{64}$`), `candidate_id` (`^sha256:[a-f0-9]{64}$`), `policy_snapshot_id` (`^sha256:[a-f0-9]{64}$`), `evidence_strategy` (`bug | feature | refactor | migration | config-docs | strict-tdd`), `selected` (array of `ChallengeType` strings with uniqueItems), `skipped` (array of `{challenge_type, reason}` objects), `reasons` (array of strings), and `budget` (object with required `max_challenges`, `mutation_budget`, `timeout_seconds`).

`challenge-result/v1` MUST require: `schema_version`, `kind`, `result_id` (`^sha256:[a-f0-9]{64}$`), `plan_id` (`^sha256:[a-f0-9]{64}$`), `candidate_id` (`^sha256:[a-f0-9]{64}$`), `challenge_type` (`revert | focal-mutation | independent-acceptance | regression-acceptance | compatibility-acceptance | test-inspection | structural-validation | behavior-equivalence | rollback`), `outcome` (`passed | failed | error`), `node_id` (string), `evidence_ids` (array of `^sha256:[a-f0-9]{64}$`), and `details` (object).

Both schemas MUST enforce `additionalProperties: false`. Both families MUST be registered in `schemas/kernel/manifest.json` and `schemas/kernel/contract-claims.json`. Challenge schemas MUST NOT validate as `evidence/v2`, `verification/v2`, `CandidateEvaluationAttestation`, or `DeliveryAuthorization`. `evidence/v2`, `verification/v2`, and K1 v1 schema bytes and `K1_SCHEMA_BASELINE` pins MUST remain byte-identical. The family MUST ship valid and invalid fixtures.

#### Scenario: Valid challenge-plan v1 payload passes validation

- GIVEN a complete `challenge-plan/v1` payload with valid candidate_id, policy_snapshot_id, selected, skipped, and budget
...
- GIVEN a challenge-plan payload omitting `budget` or containing an unknown `challenge_type` in `selected`
- WHEN validated against `challenge-plan/v1.schema.json`
- THEN validation MUST fail closed identifying the violating property

#### Scenario: Valid challenge-result v1 payload passes validation

- GIVEN a complete `challenge-result/v1` payload with valid result_id, candidate_id, outcome `passed`, and details
- WHEN validated against `challenge-result/v1.schema.json`
- THEN validation MUST succeed

#### Scenario: Challenge-result with invalid outcome fails closed

- GIVEN a challenge-result payload with `outcome: "unresolved"`
- WHEN validated against `challenge-result/v1.schema.json`
- THEN validation MUST fail closed identifying the invalid enum value
