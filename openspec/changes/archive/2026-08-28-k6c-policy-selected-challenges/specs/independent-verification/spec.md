...
The verifier MAY consume schema-valid `challenge-result/v1` and `challenge-plan/v1` records bound to the frozen `CandidateId` as complementary verification evidence.

When policy or strategy mandates challenge verification:
1. The verifier MUST verify that all `selected` challenges in the bound `ChallengePlan` have corresponding `challenge-result/v1` records with `outcome: "passed"`.
2. If any selected challenge has `outcome: "failed"` (including `COMPLACENT_TEST_DETECTED` or `TAUTOLOGICAL_TEST_DETECTED`) or `outcome: "error"`, verification MUST fail closed with `CHALLENGE_VERIFICATION_FAILED` and MUST NOT emit a `PASS` or `PASS WITH WARNINGS` verdict.
3. If budget exhaustion occurred during challenge execution, verification MUST fail closed with `CHALLENGE_BUDGET_EXHAUSTED`.

Challenge results MUST remain complementary evidence only. Challenge results MUST NOT substitute for declared strategy minimums, MUST NOT bypass MUST-walk obligation coverage, and MUST NOT grant delivery or lifecycle authorization.

#### Scenario: Successful challenge results satisfy complementary verification

- GIVEN a frozen Candidate with strategy minimums met and all MUST obligations satisfied
- AND bound `ChallengePlan` whose selected challenges all have `challenge-result/v1` records with `outcome: "passed"`
- WHEN the verifier evaluates candidate evidence
- THEN the challenge results MUST be accepted as satisfying complementary verification
- AND the verifier MAY emit `PASS` or `PASS WITH WARNINGS`

#### Scenario: Failed challenge result fails verification closed

- GIVEN a bound `ChallengePlan` with a `challenge-result/v1` record having `outcome: "failed"` due to `COMPLACENT_TEST_DETECTED`
- WHEN the verifier evaluates candidate evidence
...
- GIVEN all selected challenge results with `outcome: "passed"`
- AND missing declared strategy minimum evidence or unfulfilled MUST obligations
- WHEN the verifier evaluates candidate evidence
- THEN verification MUST fail closed
- AND challenge results MUST NOT override or substitute for missing strategy evidence
