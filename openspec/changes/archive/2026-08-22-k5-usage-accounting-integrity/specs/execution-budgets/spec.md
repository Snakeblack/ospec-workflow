...
All node and authority budgets MUST be strictly monotonically non-increasing across execution retries, recovery loops, and concurrent CAS conflict reconciliations. `ExecutionUsage` MUST be runtime-owned and extracted exclusively from `result.usage` or `result.execution_usage` emitted by `effectExecutor` for a physical execution. A physically executed effect that requires usage but returns neither field MUST fail closed with stable reason `execution-usage-required`; the runtime MUST NOT estimate usage from `input.consumed`, effect arguments, or historical journal results. The runtime MUST apply each newly reported usage delta exactly once to the state presented to CAS. If a post-effect path does not confirm that CAS, including `effect-failed`, ambiguity, reconciliation, or `cas-conflict`, the runtime MUST retain that delta as pending carry-over under `${subjectId}:${nodeId}`. Carry-over MUST be deducted exactly once from the winning head on retry. Reconciled historical or skipped completed effects MUST NOT create a new delta. Carry-over MUST cover all declared node and authority dimensions and MUST NOT leak between nodes or replenish quota.

(Previously: Runtime-owned usage and carry-over were required after CAS conflict, but successful and other post-effect exits were not required to account exactly once, and the zero-usage fallback was not forbidden.)

#### Scenario: Successful CAS accounts physical execution exactly once

- GIVEN an effect reports usage of 7 commands, 2 patches, and 120 changed lines
- WHEN its operation wins CAS
- THEN the committed state MUST deduct those values exactly once
- AND no pending carry-over MUST remain for that execution

#### Scenario: Post-effect failure retains reported usage

- GIVEN an effect physically executes, reports usage, and returns `ok: false`
- WHEN the runtime returns `effect-failed` before a successful CAS
- THEN it MUST retain the reported delta as runtime-owned carry-over
- AND MUST NOT restore the consumed budget

#### Scenario: Historical completed result cannot re-debit on repeated CAS conflicts

- GIVEN one physical effect execution reports 3 turns and the first CAS loses
- WHEN retries skip its completed journal record and a second CAS also loses
- THEN carry-over MUST remain 3 turns, not 6
- AND the executor MUST have been called exactly once

#### Scenario: Concurrent multi-writer conflict retains an exhaustive delta

- GIVEN two writers execute effects with usage across turns, commands, patches, changed lines, and effect attempts
- WHEN one writer wins CAS and the other loses
- THEN the losing writer MUST retain every reported dimension as carry-over
- AND its retry MUST deduct those dimensions without re-executing a completed effect

#### Scenario: Repair retry remains monotonic

- GIVEN a repair loop begins with 3 allocated effect attempts
- WHEN a failed physical execution causes an allowlisted repair retry
- THEN the next attempt MUST retain the prior consumption
- AND no quota MAY be replenished implicitly

#### Scenario: Missing execution usage fails closed

- GIVEN an effect-bearing execution whose result contains neither usage field
- WHEN post-effect accounting runs
- THEN the runtime MUST return `execution-usage-required`
- AND MUST NOT derive a delta from caller arguments

#### Scenario: Caller-supplied input.consumed is rejected as usage authority

- GIVEN a caller supplies fabricated `input.consumed` and the executor reports distinct usage
- WHEN the operation is accounted
- THEN only the executor-reported usage MUST affect budget
- AND `input.consumed` MUST be ignored as accounting authority

#### Scenario: Partitioned carry-over prevents budget contamination between concurrent nodes

- GIVEN node `N1` under subject `S1` retains usage after an unconfirmed CAS
- WHEN node `N2` under `S1` evaluates its budget
- THEN the pending delta MUST be keyed under `S1:N1`
- AND `N2` MUST NOT have its quota reduced

### Requirement: Zero-Delta Attempt Consumption And Monotonic Invariants {#REQ-execution-budgets-004}

Zero-delta accounting MUST be strictly bounded to effect-bearing code or file mutation operations where `effectProgress === false` and modified lines or files are zero. Such steps MUST consume one execution attempt and decrement BOTH node turns and authority `effect_attempts` monotonically, recording `zero-delta-attempt` in the journal before CAS. `lifecycleProgress` MUST NOT by itself exempt an effect-bearing mutation: a `repair` that advances lifecycle state while producing no effect progress MUST be zero-delta. Non-mutating inspections, read-only diagnostics, and terminal control transitions MUST NOT be classified as zero-delta. Decrements MUST remain monotonic across retries and carry-over.

(Previously: A `repair` reporting lifecycle advancement was exempt even when its effect made no code or file progress.)

#### Scenario: Sterile repair receives dual zero-delta penalty

- GIVEN `repair` moves lifecycle state from failed to pending but has no modified files or effect progress
- WHEN post-effect zero-delta accounting runs
- THEN node turns and authority effect attempts MUST each decrement once
- AND a durable `zero-delta-attempt` event MUST precede CAS

#### Scenario: Effect-bearing code patch with no progress receives dual penalty

- GIVEN an effect-bearing mutation has zero modified lines and `effectProgress: false`
- WHEN accounting runs
- THEN the runtime MUST record one zero-delta attempt
- AND MUST decrement both budget dimensions monotonically

#### Scenario: Read-only inspection does not consume zero-delta attempt

- GIVEN a read-only diagnostic step completes without a mutation effect
- WHEN accounting runs
- THEN it MUST NOT be classified as zero-delta
- AND effect attempts MUST NOT be dual-decremented

#### Scenario: Terminal lifecycle control is not zero-delta

- GIVEN an `escalate` or `stop` transition makes no file mutation
- WHEN post-effect accounting runs
- THEN it MUST NOT be classified as zero-delta
- AND MUST NOT consume the dual penalty

#### Scenario: Zero-delta consumption survives a CAS race

- GIVEN a zero-delta mutation loses CAS after accounting
- WHEN the operation retries against the head
- THEN carry-over MUST preserve its dual decrement exactly once
- AND MUST NOT restore either budget
