...
The system MUST provide a Go TUI that receives seven supported targets from its adapter, schedules exactly one target per run, and advances through target, model, and review steps. Back from any non-initial step MUST retain valid selections.
...
- GIVEN the user selected a target and available models
...
The adapter plan MUST declare each agent's selectability and finite supported models. The TUI MUST offer only declared choices, reject free-form or unsupported identifiers, and display inherited behavior otherwise. A missing configuration column alone MUST NOT prove model support. Antigravity MUST display inheritance and omit multi-model selection.
...
- AND the summary MUST show its effective model
...
The TUI MUST summarize the selected target and every agent's selected or inherited behavior before installation. It MUST invoke no target installer or destination write before explicit Install. Cancel, exit, and Back from summary MUST preserve or discard the plan without installation.

#### Scenario: Summary waits for Install

- GIVEN a valid plan
- WHEN its summary is displayed before Install
- THEN no installer invocation or destination write MUST occur

#### Scenario: Edit from summary

- GIVEN the summary shows an effective model
- WHEN the user chooses Back, changes an allowed model, and returns
...
The Node adapter MUST expose a machine-readable plan from canonical target and model configuration without changing `models.yaml`. On Install it MUST revalidate the plan, pass selected models as an in-memory override, and delegate only the selected target to its compatible existing `main(argv, deps)` installer seam. It MUST preserve installer outcomes and diagnostics and MUST NOT duplicate validation, transaction, rollback, manifest, or destination-write behavior.

#### Scenario: Planning is read-only

- GIVEN a caller requests an adapter plan
- WHEN it returns targets, model capabilities, and inherited states
- THEN `models.yaml` and installation destinations MUST remain unmodified

#### Scenario: Confirmed plan delegates once

- GIVEN a displayed plan remains valid at confirmation
- WHEN the user chooses Install
- THEN exactly the selected installer MUST run
- AND its outcome and diagnostics MUST reach the TUI

#### Scenario: Invalid plan is refused

- GIVEN confirmation contains a target or model absent from a fresh plan
