...
### 7.1 Generalist-First Read-Only Review {#REQ-agents-012}
...
### 7.2 Selective Specialist Dispatch with Slice-Scoped Remediation {#REQ-agents-013}
...
### 7.3 Bounded Review Lineage with Independent Correction Slices {#REQ-agents-015}
...
### 7.6 Strict TDD Evidence-Format-Gap Fast Path Contract {#REQ-agents-016}
...
### 7.8 Approved Successor Review Authority {#REQ-agents-016}

Before creating a successor review lineage, the orchestrator MUST resolve the
referenced approval by exact `id` against the change approval ledger. Acceptance
MUST depend on the resolved entry's valid source, explicit decision, and
`applies_to` scope for the requested review operation; it MUST NOT depend on an
identifier naming pattern. The transition authority MUST be one of
`new-candidate`, `new-scope`, or `new-discovery-authority`, MUST name a terminal
predecessor, and MUST carry a candidate identity distinct from that predecessor.
The predecessor MUST remain closed and MUST NOT be reopened, reset, or reused.

Missing, duplicate/reused, source-invalid, out-of-scope, or authority-mismatched
approval references MUST fail closed before any reviewer dispatch. The resolved
approval reference and transition authority MUST be retained as successor audit
data.

#### Scenario: Generic valid approval authorizes successor

- GIVEN ledger approval `4r-warning-remediation-001` has a valid source and scope
- AND a terminal predecessor and distinct candidate are supplied
- WHEN a `new-scope` successor is requested by exact approval reference
- THEN successor creation MUST be accepted without an identifier-pattern check
- AND the approval reference and authority MUST be retained

#### Scenario: Other valid ledger identifiers remain valid

- GIVEN approvals with exact ids `approval-42` and `release.scope.v1`
- AND each resolves to a valid, in-scope ledger entry
- WHEN either id is referenced for its permitted successor authority
- THEN it MUST be accepted independently of its identifier spelling

#### Scenario: Invalid approval reference fails closed

- GIVEN an approval id that is missing, duplicated, source-invalid, or outside scope
- WHEN successor creation is requested
- THEN it MUST be rejected before a generalist or specialist is dispatched
- AND the predecessor MUST remain terminal and unchanged

#### Scenario: Invalid successor authority cannot reopen predecessor

- GIVEN a terminal predecessor and an approval for an unrelated operation
- WHEN a successor request has an unallowlisted authority or the same candidate
- THEN it MUST be rejected fail closed
- AND no predecessor reviewer, finding, budget, or execution MUST be reset
