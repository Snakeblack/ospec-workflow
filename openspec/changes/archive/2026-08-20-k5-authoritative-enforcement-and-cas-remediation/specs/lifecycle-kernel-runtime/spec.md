...
Lifecycle reducers, mutation processors, and `runKernelOperation` MUST enforce strict budget monotonicity and unified exhaustion checks via `isBudgetExhausted()`. The runtime MUST evaluate budget exhaustion in preflight prior to invoking `effectExecutor`; if any node or authority quota is exhausted, execution MUST fail closed with exactly zero calls to `effectExecutor`. Across retries, recovery loops, and concurrent CAS conflict reconciliations, remaining node quotas (`turns`, `patches`, `commands`, `wall_time_minutes`, `changed_lines`, `allowed_paths`) and authority/effect limits (`effect_attempts`, `authority_mutations`, `evidence_runs`, `review_sweeps`) MUST be strictly monotonically non-increasing. When an effect is executed and the operation subsequently loses a CAS race, the consumed budget MUST NOT be restored upon re-synchronization. Reducers and runtime wrappers MUST NOT reset, restore, or silently replenish consumed budgets during state transitions or retry attempts.
(Previously: Budget monotonicity did not explicitly enforce zero effect executor invocations upon preflight budget exhaustion or budget retention across lost CAS races.)
...
#### Scenario: CAS reconciliation preserves consumed budget in next state after executed effect

- GIVEN an operation that executed an effect and encountered a CAS conflict with 4 turns consumed
- WHEN the reducer and runtime compute the next state transition against the updated head
- THEN the state MUST reflect the 4 consumed turns and decremented attempt quota
- AND MUST NOT reset the turn or attempt counter to initial values

#### Scenario: Preflight budget exhaustion halts runKernelOperation with zero effect executor calls

- GIVEN a lifecycle state where `isBudgetExhausted()` returns `exhausted: true` for node or authority quotas
- WHEN `runKernelOperation` executes
...
The lifecycle kernel runtime, transition selector (`selectTransitions`), and execution coordinator (`runKernelOperation`) MUST resolve mixed failure sets according to deterministic causal priority precedence (`environment_tooling > cas_conflict > ambiguous_effect > validation_gap > code_defect`). The runtime MUST select and emit next transitions strictly from the allowlisted transition matrix (`repair`, `replan`, `escalate`, `stop`) for the resolved primary failure code. For `code_defect` with positive remaining `effect_attempts`, the selector MUST emit `{ kind: "execute", operation: "repair" }` without degrading or renaming the operation to `recover`. For `operation: "repair"`, `args.scope` MUST be mandatory in preflight; if missing or invalid, `runKernelOperation` MUST fail closed (`repair-scope-violation`) with zero calls to `effectExecutor`. The selector MUST emit `escalate` explicitly when required and MUST NOT silently substitute `decide`. The runtime MUST process `escalate` transitions and commit them to the Authority Store via CAS as a consolidated terminal outcome.
(Previously: Selector degraded code_defect repair transitions to recover, permitted silent substitution of escalate with decide, and aborted escalate without CAS consolidation.)
...
#### Scenario: Escalate operation commits consolidated terminal status to Authority Store via CAS

- GIVEN a kernel operation executing an `escalate` transition
- WHEN `runKernelOperation` executes the operation
- THEN the runtime MUST commit the escalated terminal status to the Authority Store via CAS
- AND MUST NOT abort runtime execution prematurely

---

### Requirement: Zero-Delta Consumption And Honest Terminality {#REQ-lifecycle-kernel-runtime-027}

The lifecycle kernel runtime MUST record zero-delta attempt consumption on effect-bearing actions that produce zero semantic state advancement. When a zero-delta mutation is detected post-effect, the runtime MUST simultaneously decrement BOTH `node.turns` (or `node.budget.turns`) AND `state.authority_budget.effect_attempts`, and MUST persist a durable `zero-delta-attempt` event in the journal before committing via CAS. When any execution or authority budget reaches zero, the runtime MUST refuse normal execution transitions and transition deterministically to `escalate` or `stop`.
(Previously: Zero-delta accounting did not mandate simultaneous dual decrement of node turns and effect attempts with durable zero-delta-attempt journal event persistence.)

#### Scenario: Zero-delta effect consumption decrements both turns and attempt counters with durable event
