...
Permit issuance MUST occur only through a controlled runtime issuer that accepts a `TransitionOffer` plus exactly one of `PolicyDecision`, `HumanDecision`, or `KernelRule`, plus `expected_revision`, and returns an `OperationPermit`. The controlled issuer MUST evaluate node budget and authority effect budget exhaustion via `isBudgetExhausted()` prior to issuing an `OperationPermit`. If the associated node or authority budget is exhausted, the issuer MUST fail closed and refuse permit issuance. State-validity of the offered transition alone MUST NOT authorize issuance or mutation. Models, hosts, and public mutating entrypoints MUST NOT self-grant or auto-mint permits.
(Previously: Controlled issuer checked offer and decision without explicit preflight budget exhaustion verification before permit issuance.)

#### Scenario: Issuer produces permit from offer plus decision when budget is available
...
#### Scenario: Issuer refuses permit when node or authority budget is exhausted

- GIVEN a valid TransitionOffer and PolicyDecision
- AND the target node or authority budget is exhausted (`isBudgetExhausted()` returns `exhausted: true`)
