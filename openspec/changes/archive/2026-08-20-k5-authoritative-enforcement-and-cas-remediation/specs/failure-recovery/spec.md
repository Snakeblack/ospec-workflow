...
The runtime and transition selector MUST map resolved causal failures deterministically to explicit allowlisted recovery transitions from `{repair, replan, escalate, stop}`:
...
The taxonomies of `kind` (`execute`, `decide`, `stop`) and `operation` (`repair`, `recover`, `replan`, `escalate`, `stop`) MUST remain harmonized. When a failure category requires escalation or cannot be repaired within remaining budget quotas, the selector MUST explicitly emit `{ kind: "escalate", operation: "escalate" }` and MUST NOT silently substitute `decide`. The runtime MUST commit `escalate` transitions to the Authority Store via CAS as a consolidated terminal outcome without premature abort. Transitions not explicitly allowlisted for the resolved failure category MUST fail closed.
(Previously: Selector allowed ambiguity between recover/repair and escalate was not committed as a consolidated CAS terminal transition.)
...
#### Scenario: Escalate transition consolidates and commits via CAS
...
- AND MUST NOT abort execution prematurely prior to CAS consolidation
