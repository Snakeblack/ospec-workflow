...
1. `inv-k5-budget-monotonicity`: verifies that no execution or authority budget increases across runtime operations, concurrent multi-writer CAS race reconciliations (with at least 2 competing writers where the losing writer's executed effect budget is preserved and not restored upon retry), or repair loops under real store/CAS execution.
...
3. `inv-k5-allowlist-enforcement`: verifies that the runtime and selector strictly emit and enforce allowlisted recovery transitions (`repair`, `replan`, `escalate`, `stop`) without unallowlisted operations or silent `decide` substitutions, emitting `operation: "repair"` for `code_defect`, harmonizing `kind`/`operation` taxonomies, and committing `escalate` to CAS as a consolidated terminal outcome.
4. `inv-k5-zero-delta-consumption`: verifies that zero-delta mutations decrement both node turns and authority `effect_attempts` monotonically within `runKernelOperation` and persist a durable `zero-delta-attempt` journal event before CAS commit.
5. `inv-k5-budget-exhaustion-terminal`: verifies that unified `isBudgetExhausted()` evaluation across all 6 node and 4 authority dimensions deterministically halts execution in preflight with exactly zero calls to `effectExecutor` and forces `escalate` or `stop` in selector and runtime.
...
(Previously: Invariant checkers did not verify multi-writer concurrent CAS conflict budget monotonicity, dual zero-delta decrement with durable journal event, or repair preflight with 0 effect executor calls.)
...
#### Scenario: Budget monotonicity verified across concurrent 2-writer CAS conflict race
...
- THEN the checker for `inv-k5-budget-monotonicity` MUST verify that writer 2 retains the decremented effect budget on retry and does not restore consumed units

#### Scenario: Zero-delta checker verifies dual turns and effect attempts decrement and journal event
...
#### Scenario: Exhausted budget terminality evaluates full six-node and four-authority dimensions with zero effect executor calls
...
- THEN the checker MUST verify that the runtime and selector reject normal execution transitions with exactly zero calls to `effectExecutor` and force terminal `escalate` or `stop`
