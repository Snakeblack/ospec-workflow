...
Phase-cost emission (`scripts/hooks/subagent-stop.js`, Go mirror `internal/hooks/subagentstop.go`), Result Envelope persistence and dispatch status resolution (`persistResultEnvelope` and `resolveDispatchStatus` in `scripts/hooks/subagent-stop.js` and `internal/hooks/subagentstop.go`), CX0 context measurement persistence (`persistContextMeasurement` in `scripts/hooks/subagent-stop.js`), and bench/CX0 coverage validation (`scripts/evals/lib/benchmark.js`) MUST derive agent identity through this same resolution point (`resolveCanonicalAgent` in JS, `agentidentity.ResolveCanonicalAgent` in Go). Consumers MUST NOT embed their own raw string matching or `sdd-`-prefix conventions where the shared resolution applies. The resolution MUST NOT introduce a plugin registry or configuration surface.

(Previously: REQ-agent-identity-002 explicitly named only phase-cost emission and bench/CX0 coverage validation as consumers, omitting persistResultEnvelope, resolveDispatchStatus, and persistContextMeasurement.)
...
#### Scenario: Envelope persistence and CX0 consumers share canonical resolution

- GIVEN a dispatch with registered name `plugin-host:sdd-spec`
- WHEN `persistResultEnvelope`, `resolveDispatchStatus`, `persistContextMeasurement`, and `persistPhaseCost` resolve the agent identity
- THEN all consumers MUST obtain the same canonical agent `sdd-spec` and derived phase key `spec`
- AND host-prefixed dispatches MUST be recognized consistently across JS and Go runtimes
