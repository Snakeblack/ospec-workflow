...
A `WorkOrder` MUST be bound to a specific `SourceSnapshotId` and declare objective, allowed paths, invariants, budget, and required evidence. A `WorkResult` MUST be bound to both `WorkOrderId` and `SourceSnapshotId`, capturing unapproved worker outputs (patch/commit, execution commands, logs, exit codes, filesystem inventory). The system MUST NOT accept an raw `WorkResult` as a `Candidate` or for attestation/delivery without candidate integration and freeze.
...
The kernel MUST freeze candidate content before verification, review, attestation, or authorization. The freeze pipeline MUST canonicalize paths, incorporate `repository_id`, base tree digest, candidate tree digest, diff hash, `changed_paths_modes_digest`, and `intended_untracked_digest`. Candidate projections MUST be restricted strictly to `workspace` or `staged`. Path mode changes (e.g. 100644 vs 100755), symlink modifications, case sensitivity shifts, and untracked entries MUST alter the resulting `CandidateId`.
...
The kernel MUST evaluate candidate relations deterministically into one of four initial relations: `exact`, `changed`, `ambiguous`, or `unknown`. `exact` SHALL trigger validation reuse; `changed` SHALL trigger re-evaluation; `ambiguous` and `unknown` SHALL fail closed with `decide` or `stop`. Ambiguous selectors, unresolved path projections, or non-deterministic relation states MUST resolve to `ambiguous` or `unknown`. Advanced relations (`compatible-base-advance`, `provable-contraction`) MUST NOT be applied as default active relations.
...
Candidate Evaluation Attestations and Delivery Authorizations MUST bind to a frozen `CandidateId` and MUST NOT point directly to mutable git branches, unintegrated working trees, or raw worker results. Attempting to issue or validate an attestation or authorization against a mutable target MUST fail closed.
