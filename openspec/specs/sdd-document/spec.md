...
Because Starlight renders the frontmatter `title` as the page H1, the sync script MUST strip the source page's leading H1 (its first non-blank line when it is a `#` heading) from the transformed body — in all cases, including pages that already declare their own frontmatter `title` — so the title is never rendered twice on a page.

#### Scenario: Leading H1 is not duplicated below the rendered title

- GIVEN an `openwiki/` page starting with `# Architecture Overview`
- WHEN the sync script transforms it
- THEN the transformed body MUST NOT retain the leading `# Architecture Overview` line
- AND the remainder of the body MUST be preserved intact
...
### Requirement: Web Site Identity and Coherent Navigation {#REQ-sdd-document-019}

The generated Starlight site MUST identify itself with the target repository's name, not a generic placeholder: `astro.config.mjs` MUST derive the site `title` from the parent repository's `package.json` `name` (title-cased per hyphen segment, scope prefix dropped), falling back to the repository directory name, and only then to a generic label. The scaffold MUST NOT ship a hardcoded final title.

The sync script MUST emit a sidebar manifest (`web-doc/src/sidebar.generated.json`) on every run, and `astro.config.mjs` MUST consume it to build the sidebar: root-level wiki pages become top links with the quickstart page always first (labelled with each page's resolved title), and each wiki subdirectory becomes an autogenerated group. Groups MUST be ordered by the directory's first mention among the quickstart page's own links — the quickstart is the generator-maintained narrative index — with directories the quickstart never mentions appended alphabetically. When the manifest is missing or unreadable, `astro.config.mjs` MUST fall back to Starlight's default autogenerated sidebar rather than failing the build; a manifest write failure MUST warn and never fail `predev`/`prebuild`.

#### Scenario: Site title reflects the target repository

- GIVEN the parent repository's `package.json` declares `"name": "ospec-workflow"`
- WHEN the generated `web-doc/` site builds
- THEN the Starlight site title MUST be `Ospec-Workflow`, not `Project Documentation`

#### Scenario: Quickstart leads a narratively ordered sidebar

- GIVEN `openwiki/` has a root `quickstart.md` whose links mention `security/` before `orchestration/`
- WHEN the sync script runs and the site builds
- THEN the sidebar's first entry MUST be the quickstart top link
- AND the `security` group MUST precede the `orchestration` group

#### Scenario: Missing manifest degrades to the default sidebar

- GIVEN `src/sidebar.generated.json` is absent or unreadable
- WHEN `astro.config.mjs` loads
- THEN the build MUST proceed using Starlight's default autogenerated sidebar
