...
When user input is needed before continuing, use `AskUserQuestion` as mapped by the active target and permitted by the host. Wait for an explicit answer; never replace a gate result with an inferred conversational approval.
...
| `quality-review-gate` | After successful `sdd-verify` returns `success` |

For `quality-review-gate` (live v2) and in-flight `4r-review-gate` (schema-v1 continuation only), `scripts/lib/review-lineage.js` owns identity, one-shot lenses, frozen findings, slice budgets, targeted validation, reconciliation, and terminal outcomes; `review-gate-state.js` adapts only its `next_action`. Persist the reducer's pending mutation before dispatch. Live routes list `quality-review-gate`; `4r-review-gate` is not admitted on live/v2 config. High-risk selects all four quality domains and skips `review-change`. Each selected lens executes once; after freeze dispatch only `review-correction` for the reducer-selected active slice, passing exactly its frozen finding IDs, permitted genesis paths, immutable candidate/lineage context, and correction delta. The validator may reopen a passed slice only with explicit `impacted_slices` regression evidence naming its frozen IDs and paths; late observations remain non-blocking follow-ups. Each slice exhausts independently after three failed validations. Before any mutable action on schema-v1 state, reconcile pending/unknown work then perform additive idempotent remediation-v2 migration. Unknown outcomes require exact reconciliation; downstream gates are read-only identity checks; new blocking discovery requires an approved `new-candidate`, `new-scope`, or `new-discovery-authority` successor linked to a terminal predecessor. Never reset attempts, budget, paths, findings, or executions.
...
| Quality Review Gate Dispatch | `quality-review-gate` listed in the active route `gates` (or `4r-review-gate` only for schema-v1 continuation) | `skills/_shared/gate-4r-review.md` | When the post-verify hook point is reached (after successful `sdd-verify` returns `success`) |
