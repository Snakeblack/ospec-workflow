...
const STATE = path.join(ROOT, "scripts", "fixtures", "review-lineage", "o4-2-gen4-post-migration-state.yaml");
...
test("O4.2 frozen post-migration fixture lineage satisfies the hardened remediation-v2 gate validator", () => {
  const before = fs.readFileSync(STATE, "utf8");
  const fixture = JSON.parse(fs.readFileSync(FIXTURE, "utf8"));
  const gate = readGate(before);
  assert.equal(gate.lineage.remediation_schema_version, 2, "fixture gate must already carry the migrated remediation schema");
  assert.equal(
    typeof (gate.lineage.remediation_migration && gate.lineage.remediation_migration.legacy_failed_attempts),
    "number",
    "fixture remediation_migration must seed legacy_failed_attempts, not only legacy_used_lines",
  );
  assert.deepEqual(
    ...
    "the exact post-migration object must pass integrity checks (not throw) without any in-memory re-migration; remediation for the ready slice is still outstanding",
  );
...
test("O4.2 archived approved lineage validates for archive gate", () => {
  const archived = path.join(ROOT, "openspec", "changes", "archive", "2026-07-25-strict-tdd-evidence-remediation-fast-path", "state.yaml");
  const gate = readGate(fs.readFileSync(archived, "utf8"));
  assert.equal(gate.lineage.status, "approved");
  assert.equal(gate.lineage.terminal_reason, "all-remediation-slices-passed");
  assert.deepEqual(
    validateLineageForGate(gate.lineage, { candidate_id: gate.lineage.current_candidate_id, gate: "archive" }),
    { valid: true, code: "lineage-approved" },
  );
  assert.ok(Object.values(gate.lineage.correction_slices).every((slice) => slice.status === "passed"));
});
